...
WITH obtainedRes AS (
    ...
    RANK() OVER (PARTITION BY geoNetwork.country ORDER BY (COUNT(device.browser)+COUNT(device.operatingSystem)) DESC) AS rank
  FROM
...
SELECT country,
        ARRAY_AGG(STRUCT(operatingSystem,browser,rank)) AS country_rank
FROM 
    obtainedRes 
WHERE rank<=3
GROUP BY country
